...
Стратегия удаления:
  1. winget uninstall       — приоритетный способ (Win 10+)
  2. winreg UninstallString — fallback для legacy-приложений
"""
...
# Пути реестра с записями об установленных приложениях
...
# .exe с такими подстроками в имени — служебные, не целевое приложение
_SKIP_EXE = ("uninst", "setup", "install", "update", "crash", "squirrel")
...
class AppManager:
    ...
    def __init__(self) -> None:
        ...
        # Нормализованные индексы: "counter-strike 2" → "counterstrike2"
        # Позволяют находить игры при вводе "counter strike" / "counterstrike"
        self._exe_norm: dict[str, str] = {}  # norm_key → exe
        self._uninstall_norm: dict[str, str] = {}  # norm_key → uninstall_str
    ...
    def _run(self, cmd: list[str], timeout: int = 60) -> tuple[int, str, str]:
        ...
        communicate() гарантирует непустые stdout/stderr даже при таймауте.
        errors=replace устраняет UnicodeDecodeError от winget (UTF-8 vs CP1252).
        """
        try:
    ...
    def _build_registry_cache(self) -> None:
        ...
        def _valid_exe(p: Path, app_lower: str) -> bool:
            return (
                p.suffix.lower() == ".exe"
                and p.exists()
                and not any(s in p.stem.lower() for s in _SKIP_EXE)
        ...
        for reg_path, hive_name in _REG_UNINSTALL_PATHS:
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and not any(s in candidate.stem.lower() for s in _SKIP_EXE)
                        ):
    ...
    def _lookup_exe(self, app_name: str) -> str | None:
        ...
        # 2. Нормализованное (убирает дефисы/пробелы: "counter strike" == "counter-strike 2")
        norm = _normalize(key)
        ...
        # 3. Транслитерация: "стим" → "stim" → попробуем как norm
    ...
    def _resolve_app_name(self, app_name: str) -> str:
        ...
        Приоритет:
          ...
          2. APP_SPECIAL_LAUNCH   — нестандартный запуск (Valorant, Roblox, steam://)
          ...
          6. APP_KNOWN_PATHS      — известные пути вне реестра (Steam)
        ...
        if special := APP_SPECIAL_LAUNCH.get(key):
        ...
        for path_str in APP_KNOWN_PATHS.get(key, []):
    ...
    def _uninstall_via_winget(self, app_name: str) -> OperationResult:
        """Удаление через winget — приоритетный способ (Win 10+)."""
        ...
        # Таймаут → лаунчер (Steam/Epic) открыл диалог подтверждения
        ...
        # code == 0, но Steam-игры передают команду лаунчеру, не удаляя сразу.
    ...
    def launch_app(self, app_name: str) -> OperationResult:
        ...
        Поддерживает:
          ...
          - UAC-повышение при WinError 5 (Valorant и др.)
        ...
        try:
            ...
            time.sleep(1.0)  # GUI-процессу нужно время на инициализацию

            # Некоторые лаунчеры (Prism, Steam-игры) запускают дочерний процесс
            ...
            # WinError 5 — требуется UAC-повышение (Valorant, некоторые игры)
    ...
    def uninstall_app(self, app_name: str) -> OperationResult:
        ...
        Args:
            app_name: Название приложения (напр. "Telegram", "VLC media player").
...
if __name__ == "__main__":
    ...
    # print(am.launch_app("roblox").to_json())
